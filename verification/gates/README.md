...
| **Stack-specific (Node tooling)** | `check_openspec.py` wrapping `@fission-ai/openspec` 1.12.0 strict validation | Pin the CLI as a dev dependency. Write the adapter to run `CI=1 OPENSPEC_TELEMETRY=0 DO_NOT_TRACK=1 OPENSPEC_NO_UPDATE_CHECK=1 openspec validate --all --strict` for `openspec/**`, print the real result, exit advisory until every sidecar is accepted, and propagate failure after promotion; the capability format and transition are in `../../planning/capability-specs.md §2`, `§5`. |
...
**Deviation four (pattern-capable spec leg).** When a project ports the
pairing gate, its `spec:` leg must accept a pattern and resolve it to the
changed capability. A fixed path or an any-spec check is not equivalent:
capability A's delta never pairs a change to capability B
(`../../planning/capability-specs.md §3`, `§5`). The leg prints its real
finding but exits advisory until every capability sidecar is accepted; after
promotion the same finding is HARD.
